...
bool InputField::on_ldown(float text_ox, float mx, float my,
                           float bx, float by, float bw, float bh, int clicks)
{
...
bool InputField::on_rdown(float mx, float my,
                           float bx, float by, float bw, float bh,
                           float clamp_max_x, float clamp_max_y)
{
    if (!hit(mx, my, bx, by, bw, bh)) return false;
...
void InputField::render_ctx(SDL_Renderer *r, float mx, float my, bool ldown, bool rdown)
{
    ...
    if (act == 0)      ed.do_copy();
    else if (act == 1) ed.do_cut();
    else if (act == 2) ed.do_paste();
}
