...
void FormEditFolder::open_add(int ci, int ri, const Conn &c,
                               const std::string &schema, const std::string &parent_id)
{
  conn_idx         = ci;
  repo_idx         = ri;
  conn             = c;
...
void FormEditFolder::open_edit(int ci, int ri, const Conn &c, const std::string &schema,
...
int FormEditFolder::render(SDL_Renderer *ren, float mx, float my, bool ldown, bool rdown, int clicks)
{
  ...
  if (ldown && !any_ctx) {
    ...
    if (h_save) {
      ...
      if (name.empty()) { err_view.set("Имя папки обязательно"); return 0; }
      auto [ok, msg] = editing
          ? rename_folder(conn, schema_name, folder_id, name)
          : create_folder(conn, schema_name, parent_folder_id, name);
      if (ok) return 1;
      err_view.set(msg);
    }
  }
