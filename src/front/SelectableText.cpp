...
namespace front {
  ...
  void SelectableText::render(
      SDL_Renderer *ren, float bx, float by, float bw, float bh, float mx, float my, bool ldown, bool rdown, Clr text_clr, int clicks)
  {
    box_x = bx;
    box_y = by;
    box_w = bw;
    ...
    for (int li = 0; li < (int)lines.size(); li++) {
      ...
      if (sel_start >= 0 && sel_start != cursor) {
        ...
        if (lo <= line_end && hi > ln.start) {
          ...
          if (hi > line_end && li + 1 < (int)lines.size()) hw += g_atlas.get(' ').adv * 0.6f;
      ...
      if (ln.len > 0) text_draw_n(ren, text.c_str() + ln.start, ln.len, text_x, center_baseline(line_y, LINE_H), text_clr);
    }
