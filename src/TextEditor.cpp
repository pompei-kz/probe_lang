...
bool TextEditor::handle_key(SDL_Keycode key, SDL_Keymod mod) {
    ...
    case SDLK_BACKSPACE:
        if (sel_start>=0)  { push_undo(TxAction::Other); delete_selection(); }
                             ...
                             buf.erase((size_t)t,(size_t)(cursor-t)); cursor=t; }
        else if (cursor>0) { push_undo(TxAction::Delete);
                             int32_t p=cursor; UChar32 cp;
                             U8_PREV(reinterpret_cast<const uint8_t*>(buf.data()),0,p,cp); (void)cp;
    ...
    case SDLK_DELETE:
        if (ctrl)          { do_cut(); }
            ...
            int32_t n=cursor; UChar32 cp;
            U8_NEXT(reinterpret_cast<const uint8_t*>(buf.data()),n,(int32_t)buf.size(),cp); (void)cp;
